...
set /A top=lines+1
set "F%top%=  U" & set "F0=  A"
for /L %%i in (1,1,%cols%) do set "F%top%=!F%top%!A" & set "F0=!F0!A"
set "F%top%=!F%top%!?" & set "F0=%F0%U"
set "F-1=  Level: 1" & set "Level=1"
set "F-2=   Rows: 0" & set "Rows=0"
set "F-3=  Score: 0" & set "Score=0"
for /L %%i in (1,1,%cols%) do set "blk=!blk!U"
